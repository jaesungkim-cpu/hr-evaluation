...
import { Upload, Plus, X, Save, GitBranch, FileSearch } from 'lucide-react';
...
export default function AdminPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setEditingEmployee(null); alert('저장되었습니다');
    } catch (e) { console.error(e); alert('오류가 발생했습니다'); } finally { setSaving(false); }
  };
  ...
  return (
    <div className="container mx-auto py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <Link href="/admin/upload" className="flex items-center space-x-4 p-6 bg-secondary bg-opacity-10 border-2 border-secondary rounded-lg hover:bg-opacity-20 transition"><Upload className="text-secondary" size={32}/><div><h3 className="font-bold text-primary">평가 파일 업로드</h3><p className="text-sm text-gray-600">업적기술서/의견서/평가표</p></div></Link>
        <Link href="/admin/review" className="flex items-center space-x-4 p-6 bg-green-50 border-2 border-green-300 rounded-lg hover:bg-green-100 transition"><FileSearch className="text-green-600" size={32}/><div><h3 className="font-bold text-primary">평가 데이터 조회</h3><p className="text-sm text-gray-600">업로드된 평가 내용 조회</p></div></Link>
        <button onClick={() => {}} className="flex items-center space-x-4 p-6 bg-light border-2 border-gray-200 rounded-lg hover:border-secondary transition"><Plus className="text-gray-400" size={32}/><div className="text-left"><h3 className="font-bold text-primary">새 구성원 추가</h3><p className="text-sm text-gray-600">새 구성원 정보 등록</p></div></button>
      </div>
      ...
      {editingEmployee&&(
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4"><div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="p-6 space-y-6">
            <div><h3 className="text-sm font-bold text-gray-500 uppercase mb-3">기본 정보</h3><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div><label className="block text-sm font-medium text-gray-700 mb-1">이름</label><input type="text" value={editForm.name||''} onChange={(e)=>setEditForm({...editForm,name:e.target.value})} className="w-full px-3 py-2 border border-gray-300 rounded-lg"/></div><div><label className="block text-sm font-medium text-gray-700 mb-1">사번</label><input type="text" value={editForm.employee_number||''} onChange={(e)=>setEditForm({...editForm,employee_number:e.target.value})} className="w-full px-3 py-2 border border-gray-300 rounded-lg"/></div><div className="md:col-span-2"><label className="block text-sm font-medium text-gray-700 mb-1">이메일</label><input type="email" value={editForm.email||''} onChange={(e)=>setEditForm({...editForm,email:e.target.value})} className="w-full px-3 py-2 border border-gray-300 rounded-lg"/></div></div></div>
            <div><h3 className="text-sm font-bold text-gray-500 uppercase mb-3">조직 정보</h3><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div><label className="block text-sm font-medium text-gray-700 mb-1">본부</label><input type="text" value={editForm.department||''} onChange={(e)=>setEditForm({...editForm,department:e.target.value})} className="w-full px-3 py-2 border border-gray-300 rounded-lg"/></div><div><label className="block text-sm font-medium text-gray-700 mb-1">팀</label><input type="text" value={editForm.team||''} onChange={(e)=>setEditForm({...editForm,team:e.target.value})} className="w-full px-3 py-2 border border-gray-300 rounded-lg"/></div><div><label className="block text-sm font-medium text-gray-700 mb-1">직무</label><input type="text" value={editForm.position||''} onChange={(e)=>setEditForm({...editForm,position:e.target.value})} className="w-full px-3 py-2 border border-gray-300 rounded-lg"/></div><div><label className="block text-sm font-medium text-gray-700 mb-1">직책</label><input type="text" value={editForm.title||''} onChange={(e)=>setEditForm({...editForm,title:e.target.value})} className="w-full px-3 py-2 border border-gray-300 rounded-lg"/></div></div></div>
            <div><h3 className="text-sm font-bold text-gray-500 uppercase mb-3">평가 설정</h3><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div><label className="block text-sm font-medium text-gray-700 mb-1">평가그룹</label><select value={editForm.group_type||'팀원'} onChange={(e)=>setEditForm({...editForm,group_type:e.target.value})} className="w-full px-3 py-2 border border-gray-300 rounded-lg"><option value="팀원">팀원</option><option value="팀장급">팀장급</option></select></div><div><label className="block text-sm font-medium text-gray-700 mb-1">역할</label><select value={editForm.role||'employee'} onChange={(e)=>setEditForm({...editForm,role:e.target.value})} className="w-full px-3 py-2 border border-gray-300 rounded-lg"><option value="employee">구성원</option><option value="first_evaluator">1차평가자</option><option value="second_evaluator">2차평가자</option><option value="ceo">CEO</option><option value="admin">관리자</option></select></div><div className="md:col-span-2"><label className="flex items-center space-x-3 cursor-pointer"><input type="checkbox" checked={editForm.is_evaluated||false} onChange={(e)=>setEditForm({...editForm,is_evaluated:e.target.checked})} className="w-5 h-5"/><span className="text-sm font-medium text-gray-700">평가대상</span></label></div></div></div>
            <div><h3 className="text-sm font-bold text-gray-500 uppercase mb-3">평가자 지정</h3><div className="grid grid-cols-1 gap-4"><div><label className="block text-sm font-medium text-gray-700 mb-1">1차평가자</label><select value={editForm.first_evaluator_id||''} onChange={(e)=>setEditForm({...editForm,first_evaluator_id:e.target.value})} className="w-full px-3 py-2 border border-gray-300 rounded-lg"><option value="">선택안함</option>{employees.filter((e)=>['first_evaluator','second_evaluator','ceo','admin'].includes(e.role)).map((e)=>(<option key={e.id} value={e.id}>{e.name} ({e.title||e.department})</option>))}</select></div><div><label className="block text-sm font-medium text-gray-700 mb-1">2차평가자</label><select value={editForm.second_evaluator_id||''} onChange={(e)=>setEditForm({...editForm,second_evaluator_id:e.target.value})} className="w-full px-3 py-2 border border-gray-300 rounded-lg"><option value="">선택안함</option>{employees.filter((e)=>['second_evaluator','ceo'].includes(e.role)).map((e)=>(<option key={e.id} value={e.id}>{e.name} ({e.title||e.department})</option>))}</select></div></div></div>
          </div>
          <div className="flex items-center justify-end space-x-3 p-6 border-t border-gray-200 sticky bottom-0 bg-white rounded-b-xl"><button onClick={()=>setEditingEmployee(null)} className="px-6 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">취소</button><button onClick={handleSave} disabled={saving} className="flex items-center space-x-2 px-6 py-2 bg-secondary text-white rounded-lg hover:bg-opacity-90 disabled:opacity-50"><Save size={16}/><span>{saving?'저장중...':'저장'}</span></button></div>
        </div></div>
      )}
    </div>
  );
