...
interface OrgNode {
  ...
  name: string;
  level: 'ceo' | '본부' | '실' | '팀';
  leader: EmployeeWithDivision | null;
  members: EmployeeWithDivision[];
  children: OrgNode[];
  members_for_edit: EmployeeWithDivision[];
}
...
export default function OrgChartPage() {
  ...
  const orgTree = useMemo(() => {
    ...
    const departmentMap = new Map<string, EmployeeWithDivision[]>();
    ...
    departmentMap.forEach((deptEmps, deptName) => {
      const divisionMap = new Map<string|null, EmployeeWithDivision[]>();
      ...
      Array.from(divisionMap.entries()).filter(([d])=>d!==null).sort(([a],[b])=>(a&&b?a.localeCompare(b):0)).forEach(([divName, divEmps]) => {
        if (!divName) return;
        const teamMap = new Map<string|null, EmployeeWithDivision[]>();
      ...
      const dtMap = new Map<string|null, EmployeeWithDivision[]>();
    ...
    <div className="container mx-auto py-8">
      ...
      {editingNode&&(
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4"><div className="bg-white rounded-xl shadow-xl w-full max-w-lg">
          ...
          <div className="p-6 space-y-6"><div><label className="block text-sm font-bold text-gray-700 mb-3">새 리더 선택</label><select value={editLeaderId} onChange={(e)=>setEditLeaderId(e.target.value)} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-secondary"><option value="">-- 선택 --</option>{employees.filter((e)=>e.id!==editingNode.leader?.id).map((e)=>(<option key={e.id} value={e.id}>{e.name} ({e.title||e.position||'직책없음'}) - {e.email}</option>))}</select></div>
